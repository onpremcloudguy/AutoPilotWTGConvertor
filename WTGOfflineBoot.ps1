...
if (!(Test-Path -Path $apfilepath)) {new-item -Path $apfilepath -ItemType Directory}
$nugetver = (Get-PackageProvider -name "NuGet").version
if ($nugetver.major -lt 2 -and $nugetver.Minor -lt 8) {Install-PackageProvider -name "nuget" -ForceBootstrap -Force | Out-Null}
if ((Get-InstalledScript -Name "get-windowsautopilotinfo").version -lt 1.3) {Install-Script -Name "get-windowsautopilotinfo" -Force}
if ((Get-ExecutionPolicy).ToString() -ne "Bypass") {Set-ExecutionPolicy -ExecutionPolicy Bypass -Force -Scope CurrentUser}
...
elseif ($disk.PartitionStyle -eq "GPT") {
    ...
    if ($ospar.size -lt 64Gb) {$ospar = $disk | New-Partition -UseMaximumSize -AssignDriveLetter}
